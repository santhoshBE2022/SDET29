...
public class TC_71_SearchTheOrganizationbyAssignedtoTest 
{
		...
		// getting ORGANISATION value from excel sheet & entering to web page
		 ...
		 driver.findElement(By.xpath("(//a[text()='Organizations'])[2]")).click();
		 driver.findElement(By.xpath("//input[@class='txtBox']")).sendKeys("Administrator");
		 Thread.sleep(4000);
		 WebElement DD = driver.findElement(By.xpath("//select[@id='bas_searchfield']"));
		 Select s = new Select(DD);
		 s.selectByVisibleText("Assigned To");
		 
		 /* Validating the successful search result by trying to identify presence of "No Organization Found" message
		 ...
		 WebElement check = driver.findElement(By.xpath("//a[text()='qwerty']"));
		 String checkText = check.getText();
		 System.out.println("checkText; " +checkText);
		 //check.getAttribute(OrgName)
	 }
